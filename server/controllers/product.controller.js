...
const listLatest = async (req, res) => {
  try {
    let products = await Product.find({}).sort('-created').limit(5).populate('shop', '_id name').select('-image').exec()
    products = products.map(product => {
      const obj = product.toObject()
      if (!obj.shop || !obj.shop.name) {
        obj.shop = { name: obj.shopName || 'Unknown' }
      }
      if (obj.imageUrl) {
