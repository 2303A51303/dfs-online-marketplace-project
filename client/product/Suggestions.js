            ...
            return <span key={i}>
              <Card className={classes.card}>
                <CardMedia
                  ...
                  image={item.image || '/api/product/image/'+item._id}
                  title={item.name}
                />
                <div className={classes.details}>
                  <CardContent className={classes.content}>
                    {item.image ? (
                      <Typography variant="h3" component="h3" className={classes.productTitle} color="primary">{item.name}</Typography>
                    ) : (
                    ...
                    <Typography type="subheading" className={classes.subheading}>
                      <Icon className={classes.icon}>shopping_basket</Icon> {(item.shop && item.shop.name) || item.shopName || 'Unknown'}
                    </Typography>
                  ...
                  <div className={classes.controls}>
                    <Typography type="subheading" component="h3" className={classes.price} color="primary">₹ {item.price}</Typography>
                      <span className={classes.actions}>
                        {item.image ? (
                          <IconButton color="secondary" dense="dense" disabled>
